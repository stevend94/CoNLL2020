...
tf.flags.DEFINE_string('mode', 'eval',
                       'One of [sample, eval, dump_emb, dump_lstm_emb, get_data]. '
                       '"get_data" function gets downloads lm_1b data.'
...
github_url = 'http://download.tensorflow.org/models/LM_LSTM_CNN'

def get_data():
    # Function used to download data for lm_1b

    # Get vocabulary
    data = requests.get(github_url + '/vocab-2016-09-10.txt', stream = True)
    with open('data/vocab-2016-09-10.txt', 'wb') as f:
        for chunk in data.iter_content(chunk_size = 1024):
            if chunk:
                f.write(chunk)

    # Get graph
    data = requests.get(github_url + '/graph-2016-09-10.pbtxt', stream = True)
    with open('data/graph-2016-09-10.pbtxt', 'wb') as f:
        for chunk in data.iter_content(chunk_size = 1024):
            if chunk:
                f.write(chunk)

    # Get shards
    shards_text = ['base', 'char-embedding', 'lstm']
    for num in range(9):
        shards_text.append('softmax' + str(num))
    for shard in shards_text:
        data = requests.get(github_url + '/all_shards-2016-09-10/ckpt-' + shard, stream = True)
        with open('data/ckpt-' + shard, 'wb') as f:
            for chunk in data.iter_content(chunk_size = 1024):
                if chunk:
                    f.write(chunk)
...
def _EvalModel(dataset):
  ...
  """
  sess, t, _ = _LoadModel(FLAGS.pbtxt, FLAGS.ckpt)
...
def _DumpEmb(vocab):
  ...
  sess, t, vocab = _LoadModel(FLAGS.pbtxt, FLAGS.ckpt)

  softmax_weights = sess.run(t['softmax_weights'])
  softmax_bias = sess.run(t['softmax/b:0'])
    
  fname = FLAGS.save_dir + '/embeddings_softmax.npy'
  with tf.gfile.Open(fname, mode='w') as f:
    np.save(f, softmax_weights[:MAX_VOCAB])
  sys.stderr.write('Finished softmax weights\n')

  fname = FLAGS.save_dir + '/softmax_bias.npy'
  with tf.gfile.Open(fname, mode='w') as f:
    np.save(f, softmax_BIAS[:MAX_VOCAB])
  sys.stderr.write('Finished softmax bias\n')

  # all_embs = np.zeros([vocab.size, 1024])
  all_embs = np.zeros([MAX_VOCAB, 1024])
  for i in xrange(MAX_VOCAB):
    input_dict = {t['inputs_in']: inputs,
                  t['targets_in']: targets,
                  t['target_weights_in']: weights}
    if 'char_inputs_in' in t:
  ...
  # save words 
  words= [vocab.id_to_word(i) for i in range(top_words)]
  with open('data/words.txt', 'a') as f:
    for word in words:
      f.write(word + '\n')
